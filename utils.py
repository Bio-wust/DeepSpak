...
from mymodel.preprocess import pca
...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='emb_pca', random_seed=2020):
    ...
    x_vec = ro.FloatVector(X.flatten(order="F"))
    rX = ro.r["matrix"](x_vec, nrow=X.shape[0], ncol=X.shape[1])

    # ---- call mclust with named args in R wrapper ----
    mclust_fun = ro.r("""
    function(x, G, model){
    suppressPackageStartupMessages(library(mclust))
    Mclust(data=x, G=G, modelNames=model)
    }
    """)
    res = mclust_fun(rX, num_cluster, modelNames)
...
def clustering(adata, n_clusters=7, key='emb', add_key='SpatialGlue', method='mclust', start=0.1, end=3.0, increment=0.01, use_pca=False, n_comps=20, random_seed=42):
    """\
    ...
    random_seed : int, optional
        Random seed for reproducible clustering. The default is 42.
    ...
    if use_pca:
       ...
       rep_data = adata.obsm[key]
       temp_adata = anndata.AnnData(X=rep_data)
       feat_pca = pca(temp_adata, n_comps=n_comps)
       adata.obsm[key + '_pca'] = feat_pca
